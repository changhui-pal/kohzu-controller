...
struct MotorController::Impl {
    Impl(std::shared_ptr<kohzu::comm::ITcpClient> tcp,
        ...
        : tcpClient(std::move(tcp)), dispatcher(std::move(disp)), stopRequested(false), cbWorkerRunning(false) {}
    ...
    static std::string makeKey(const std::string& cmd, const std::vector<std::string>& params) {
        int axis = -1;
        if (!params.empty()) {
            try { axis = std::stoi(params[0]); } catch (...) { axis = -1; }
        }
...
MotorController::MotorController(std::shared_ptr<kohzu::comm::ITcpClient> tcp,
                                 std::shared_ptr<kohzu::protocol::Dispatcher> dispatcher)
    : impl_(std::make_unique<Impl>(tcp, dispatcher)) {}

MotorController::~MotorController() { try { stop(); } catch (...) {} }
...
void MotorController::start() {
    ...
    if (!impl_->writer) {
        ...
        impl_->writer->registerErrorHandler([this](std::exception_ptr ep){
            try { if (ep) std::rethrow_exception(ep); } catch (const std::exception& e) { std::cerr << "[MotorController] writer error: " << e.what() << "\n"; } catch (...) { std::cerr << "[MotorController] writer unknown error\n"; }
            if (impl_->dispatcher) impl_->dispatcher->cancelAllPendingWithException("Writer error");
        });
        impl_->writer->start();
    }
    ...
    impl_->tcpClient->registerRecvHandler([this](const std::string& line) {
        ...
        if (!pr.valid) {
            std::cerr << "[MotorController] parse failed: " << line << std::endl;
            return;
        }
        auto &resp = pr.resp;
        ...
        } catch (const std::exception& e) { std::cerr << "[MotorController] dispatcher tryFulfill threw: " << e.what() << "\n"; }
        catch (...) { std::cerr << "[MotorController] dispatcher tryFulfill unknown\n"; }
        if (!matched) {
            try {
                if (impl_->dispatcher) impl_->dispatcher->notifySpontaneous(resp);
            } catch (...) { std::cerr << "[MotorController] notifySpontaneous threw\n"; }
        }
    });
    ...
    impl_->cbWorkerThread = std::thread([this]() {
        while (!impl_->stopRequested.load()) {
            Impl::CallbackTask task;
            {
                ...
                impl_->cbCv.wait_for(lk, std::chrono::milliseconds(100), [this](){ return impl_->stopRequested.load() || !impl_->cbQueue.empty(); });
            ...
            if (task.cb) {
                try {
                    ...
                    try { if (task.cb) task.cb(resp, eptr); } catch (...) {}
                }
            }
...
void MotorController::stop() {
    ...
    if (impl_->dispatcher) impl_->dispatcher->cancelAllPendingWithException("MotorController stopped");
...
std::future<MotorController::Response> MotorController::sendAsync(const std::string& cmd, const std::vector<std::string>& params) {
    if (!impl_->writer) throw std::runtime_error("Writer not started");
...
MotorController::Response MotorController::sendSync(const std::string& cmd, const std::vector<std::string>& params, std::chrono::milliseconds timeout) {
    ...
    if (fut.wait_for(timeout) == std::future_status::ready) {
        ...
        std::string key = Impl::makeKey(cmd, params);
        impl_->dispatcher->removePendingWithException(key, "timeout");
...
void MotorController::registerSpontaneousHandler(kohzu::protocol::Dispatcher::SpontaneousHandler h) {
