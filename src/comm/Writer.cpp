...
Writer::~Writer() {
    try {
        ...
        std::cerr << "[Writer::~] stop error: " << e.what() << "\n"; // 수정: dtor 예외 로그
    } catch (...) {
        std::cerr << "[Writer::~] stop unknown error\n";
    }
}
...
void Writer::stop(bool flush) {
    ...
    if (workerThread_.joinable()) {
        try {
            ...
            std::cerr << "[Writer::stop] thread join error: " << e.what() << "\n"; // 수정: join 예외 처리
        } catch (...) {
            std::cerr << "[Writer::stop] thread join unknown error\n";
        }
    }
...
void Writer::workerLoop() {
    ...
    for (;;) {
        ...
        try {
            ...
            // call handler on worker thread (synchronous)
            {
                ...
                if (errorHandler_) {
                    try {
                        ...
                        std::cerr << "[Writer] errorHandler threw: " << ex.what() << "\n"; // 수정: 로그 추가
                    } catch (...) {
                        std::cerr << "[Writer] errorHandler unknown exception\n";
                    }
                }
            }
            ...
            std::cerr << "[Writer] sendLine unknown error\n"; // 수정: catch (...) 로그
            stopRequested_.store(true);
            cv_not_empty_.notify_all();
            break;
        }
